...
def collect(idx, N):
    ...
    imgs_observation = torch.zeros(N, 3, 128, 128, dtype=torch.uint8)
    imgs_result = torch.zeros(N, 3, 128, 128, dtype=torch.uint8)

    try:
        for i in range(N):
            action_id = np.random.randint(4)
            obj_pos, pixels = env.state()
            imgs_observation[i] = pixels
            env.step(action_id)
            obj_pos, pixels = env.state()
            positions[i] = torch.tensor(obj_pos)
            actions[i] = action_id
            imgs_result[i] = pixels
            env.reset()
            print(f"Process {idx}: {i}/{N}") if i % 10 == 0 else None
    except KeyboardInterrupt:
        pass

    print(f"Process {idx} saving data")
    torch.save(positions, f"data/positions_{idx}.pt")
    torch.save(actions, f"data/actions_{idx}.pt")
    torch.save(imgs_observation, f"data/imgs_observation_{idx}.pt")
    torch.save(imgs_result, f"data/imgs_result_{idx}.pt")


if __name__ == "__main__":
    processes = []
    for i in range(1):
        p = Process(target=collect, args=(i, 1000))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
